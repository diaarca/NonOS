...
//----------------------------------------------------------------------
//  GetNewThreadId
//      Define the new unique id of the thread.
//
//  Returns:
//      The id of the new thread.
//
//----------------------------------------------------------------------
Semaphore *semTID = new Semaphore("ThreadId sem", 1);
int GetNewThreadId(Thread* t)
{
    semTID->P();
    static int lastId = 0;
    int tid = tidMap->FindStart(lastId);
    if (tid != -1){
        threads[tid] = t;   
    }
    lastId = tid + 1;
...
int Thread::GetUserThreadId() { 
    if (GetThreadInfoFromTid(threadId)){
        return  GetThreadInfoFromTid(threadId)->userthread_id;
    }
    return -1; 
}
