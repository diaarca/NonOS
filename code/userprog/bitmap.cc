...
//----------------------------------------------------------------------
// BitMap::FindStart
//      Return the number of the first bit which is clear after the startPoint.
//      As a side effect, set the bit (mark it as in use).
//      (In other words, find and allocate a bit.)
//      Goes back to test from 0 when it reach the maximal value
//      If no bits are clear, return -1.
//----------------------------------------------------------------------

int BitMap::FindStart(int startPoint)
{
    for(int i = 0; i < numBits; i++)
    {
        if(!Test((startPoint + i) % numBits))
        {
            Mark((startPoint + i) % numBits);
            return (startPoint + i) % numBits;
        }
    }
    return -1;
}
